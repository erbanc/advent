...
class SeedRange(val seed: Long, val range: Long)
...
var seeds: MutableList<SeedRange> = mutableListOf()
...
doc[0].split(":")[1].split(" ").filter { it.isNotBlank() }.forEach {
    val start = it.split("-")[0].toLong()
    val range = it.split("-")[1].toLong()
...
seeds.forEach { s ->
    ...
    for (seedNb in s.seed..<s.seed + s.range) {
        val location =
                                        ...
                                        waterToLight.findDestination(
                                                fertilizerToWater.findDestination(
                                                        soilToFertilizer.findDestination(
                                                                seedToSoil.findDestination(seedNb)
                                                        )
                                                )
                                        )
                                )
