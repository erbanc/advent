...
class CalibrationEquation(var testValue: Long, var calibrationNumbers: List<Long>) {
    ...
    fun isTrue(tempValue: Long = 0, deepIndex: Int = 0): Boolean {
        ...
        // explore + option
        val isTruePlusOption = isTrue(
            if (deepIndex == 0) calibrationNumbers[0]
            else tempValue + calibrationNumbers[deepIndex],
            deepIndex + 1
        )

        // explore * option
        val isTrueMultiplyOption = isTrue(
            if (deepIndex == 0) calibrationNumbers[0]
            else tempValue * calibrationNumbers[deepIndex],
            deepIndex + 1
        )

        // explore concatenation option
        val isTrueConcatOption = isTrue(
            if (deepIndex == 0) calibrationNumbers[0]
            else (tempValue.toString() + calibrationNumbers[deepIndex].toString()).toLong(),
            deepIndex + 1
        )

        return isTruePlusOption || isTrueMultiplyOption || isTrueConcatOption
    }
}
